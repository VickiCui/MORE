...
class CommongenDataset(Dataset):
    def __init__(
        ...
        for i, (input_data, target_data) in tqdm(enumerate(zip(input_datas, target_datas))):
            ...
            res = {
                "inputs": source,
                "labels": target,
                "texts": concepts2texts.get(concepts, []),
            }
    ...
    def __getitem__(self, i):
        ...
        contexts = []
        for i in range(self.args.text_num):
            if i < len(item["texts"]):
                text = item["texts"][i]
                text_ra_tokens = self.tokenizer.tokenize(text)
                text_ra_tokens = text_ra_tokens[:512-len(input_tokens)-len(a)-len(b)-self.args.prompt_length - 1]
            else:
                text_ra_tokens = self.tokenizer.tokenize("none")

            input_tokens_ = a + text_ra_tokens + b + input_tokens + [self.tokenizer.eos_token]
            contexts.append(self.tokenizer.convert_tokens_to_ids(input_tokens_))

        item_ = {
            "input_ids": _torch_collate_batch(contexts, self.tokenizer),

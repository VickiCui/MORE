    ...
    def _compute(self, predictions, references, rouge_types=None, use_agregator=True, use_stemmer=False):
        if "|" not in references[0]:
            ...
            for refs, pred in zip(references, predictions):
                max_score = {'rouge2': Score(precision=0, recall=0, fmeasure=0)}
                for ref in refs:
                    score = scorer.score(ref, pred)
                    if score["rouge2"].fmeasure > max_score["rouge2"].fmeasure:
                        max_score = score
                if use_agregator:
                    aggregator.add_scores(max_score)
                else:

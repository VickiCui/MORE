...
class MoreBLIP2(PreTrainedModel):
    def __init__(
        self,
        config,
        args,
        ...
        self.backbone = Blip2ForConditionalGeneration.from_pretrained(
            backbone_model)
        self.generation_config = self.backbone.language_model.generation_config
        ...
        self.prompt_tokens = InputPrompts(prompt_length, self.backbone.language_model.config.hidden_size)

        ########## former ###########
        self.integrator_embedding, self.integrator, self.former, self.query_tokens, self.lm_proj = None, None, None, None, None
    ...
    def init_Qformer(self, integrator_path, num_query_token, num_integrator_layers=2, integrator_attn_list=[["self", "cross"], ["self", "cross"]], num_former_layers=1, former_attn_list=[["cross"]]):
        ...
        integrator_embedding = self.backbone.language_model.get_input_embeddings()
        ...
        lm_proj = nn.Linear(
            former_config.hidden_size, self.backbone.language_model.config.hidden_size
        )
    ...
    def forward_image(
        ...
        if pixel_values is not None and image_inputs is None:
            #pixel_values: B, N, ~
            B, N = pixel_values.size()[:2]
            pixel_values = pixel_values.view([B*N, pixel_values.size(2), pixel_values.size(3), pixel_values.size(4)])
            ...
            query_outputs = self.backbone.qformer(
                query_embeds=query_tokens,
                encoder_hidden_states=image_embeds,
                encoder_attention_mask=image_attention_mask,
            )
            query_output = query_outputs[0]
            image_inputs = image_inputs.view([B, N, 32, -1])
            image_inputs_attention_mask = torch.ones(image_inputs.size()[:-1], dtype=torch.long, device=device)
        ...
        if image_inputs is not None:
            with self.maybe_autocast():
                ...
                image_embeds = self.backbone.language_projection(image_embeds)
    ...
    def forward_text(
        ...
        if input_ids is not None and text_inputs is None:
            text_inputs = self.backbone.language_model.get_input_embeddings()(input_ids)
    ...
    def forward_query(
        ...
        if len(embeds) > 0:
            if query_tokens is not None:
                ...
                retrieved_inputs_t5 = embeds
                retrieved_atts_t5 = atts
    ...
    def forward(
        ...
        ########## image ##########
        image_embeds, image_atts = self.forward_image(pixel_values, image_inputs, image_inputs_attention_mask, device)

        ########## text ##########
        text_embeds, text_atts = self.forward_text(text_input_ids, text_inputs, text_inputs_attention_mask, device)
        ...
        ########## lm ##########
        with self.maybe_autocast():
            inputs_embeds = self.backbone.language_model.get_input_embeddings()(input_ids)
            ...
            if self.backbone.config.use_decoder_only_language_model:
    ...
    def generate(
        ...
        **generate_kwargs,
        ...
        with self.maybe_autocast():
            ...
            outputs = self.backbone.language_model.generate(
                inputs_embeds=inputs_embeds,
                attention_mask=attention_mask,
                **generate_kwargs,
            )
    ...
    def get_input_embeddings(self):
        return self.backbone.language_model.get_input_embeddings()

    def set_input_embeddings(self, value):
        self.backbone.language_model.set_input_embeddings(value)

    def set_output_embeddings(self, new_embeddings):
        self.backbone.language_model.set_output_embeddings(new_embeddings)

    def get_output_embeddings(self) -> nn.Module:
        return self.backbone.language_model.get_output_embeddings()

    def get_encoder(self):
        return self.backbone.language_model.get_encoder()

    def get_decoder(self):
        return self.backbone.language_model.get_decoder()

    def _tie_weights(self):
        if not self.backbone.config.use_decoder_only_language_model:
            self.backbone.language_model.encoder.embed_tokens = self.language_model.shared
            self.backbone.language_model.decoder.embed_tokens = self.language_model.shared